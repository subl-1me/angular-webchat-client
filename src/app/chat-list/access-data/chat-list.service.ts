...
export class ChatListService {
  ...
  private chatListener(): void {
    this.socketService.chatListener.subscribe((response: any) => {
      ...
      let isUserParticipant = chatParticipants.find(
      ...
      let chatInList = this.chats.find(
        (_chat) => _chat._id === response.chat._id
      );
      if (!chatInList) {
        response.chat.participants = response.chat.participants.filter(
          (_user: User) => _user._id !== this.userId
        );
  ...
  private getRequest(): Observable<any> {
    return this.http
      .get(this.API_URL + 'chat', {
        headers: this.headers,
        params: { userId: this.userId },
      })
      .pipe(tap(), catchError(this.handleError<Chat[]>('getChats')));
  }
